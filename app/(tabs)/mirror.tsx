...
// ── shared pieces ─────────────────────────────────────────────────────────────
function Hero({ hero, theme, s }: any) {
  return (
    <View style={[s.card, s.heroCard]}>
      <Text style={s.mono}>YOU ARE BECOMING</Text>
      <Text style={s.archetype}>{hero.archetype}</Text>
      <Text style={[s.body, { marginTop: 10 }]}>{hero.becoming}</Text>
      <View style={[s.outlookPill, { marginTop: 14 }]}>
        <Text style={s.outlookText}>OUTLOOK · TRENDING {hero.outlook.toUpperCase()}</Text>
      </View>
      <View style={s.chipRow}>
        <Chip s={s} v={`${hero.chips.momentum}`} k="MOMENTUM" />
        <Chip s={s} v={`${hero.chips.consistency}%`} k="CONSISTENCY" />
        <Chip s={s} v={`${hero.chips.streak}d`} k="STREAK" />
      </View>
    </View>
  );
}
const Chip = ({ s, v, k }: any) => (
  <View style={s.chip}><Text style={s.chipV}>{v}</Text><Text style={s.chipK}>{k}</Text></View>
);
